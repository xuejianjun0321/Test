...
public class DesensitizedUtils {
     ...
     * @return
     */
    public static String idCardNum2(String id) {
        if (StringUtils.isBlank(id)) {
            return "";
        }

        String left = StringUtils.left(id, 3);
        String right = StringUtils.right(id, 4);

        String repeat = StringUtils.repeat("*", id.length() - left.length() - right.length());

        return left + repeat + right;
    }

    public static void main(String[] args) {
        System.out.println(DesensitizedUtils.idCardNum2("330825198909073513344323235"));
    }
